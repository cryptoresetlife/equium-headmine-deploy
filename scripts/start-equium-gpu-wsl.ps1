...
$useFullGpu = $FullGpu.IsPresent -or ($env:EQUIUM_GPU_FULL -match "^(1|true|yes)$")
...
Write-Host "Starting Equium GPU miner in WSL..."
...
if ($useFullGpu) { Write-Host "Mode: full GPU" } else { Write-Host "Mode: hybrid GPU leaves + CPU Wagner" }

$mineCommand = @()
if ($Backend) {
  $mineCommand += @("env", "EQUIUM_BACKEND=$Backend")
}
$mineCommand += @(
  "./target/release/equium-gpu-miner",
  "mine",
  "--rpc-url", $RpcUrl,
  "--keypair", $Keypair,
  "--threads", "$Threads",
